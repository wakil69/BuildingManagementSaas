...
const validationSchemaAddCoworkingPrix = yup
  .object({
    prix: yup
      .array()
      .of(
        yup.object().shape({
          prix_coworking: yup.number().min(0).optional(),
        })
      )
...
export default function AddPrixCoworking({
  setIsOpen,
  batimentID,
  potentialDateDebut,
}: {
  ...
  const {
    ...
    resolver: yupResolver(validationSchemaAddCoworkingPrix),
    defaultValues: {
      prix: [{ prix_coworking: 0 }],
      prix_type: "coworking",
    ...
    <Card>
      <CardContent>
        <Typography variant="h6" id="modal-title" gutterBottom>
          Ajouter un nouveau groupe de prix - Formule Coworking
        </Typography>
                ...
                {fields.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      <TextField
                        type="number"
                        {...register(`prix.${index}.prix_coworking` as const)}
                        defaultValue={item.prix_coworking}
                      />
                    </TableCell>
                  </TableRow>
                ))}
