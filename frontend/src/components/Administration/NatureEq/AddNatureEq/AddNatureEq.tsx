...
export default function AddNatureEq() {
  const [message, setMessage] = useState("");
  const { natureEquipements, isLoadingNatureEq } = useEqType();
  const queryClient = useQueryClient();

  const validationSchemaNatureEq = Yup.object().shape({
    name: Yup.string()
      .required("La nature est requis.")
      .test("unique-name", "La nature existe déjà.", (value) => {
        if (!value || !natureEquipements) return true;
        return !natureEquipements.some(
          (form) =>
  ...
  const {
    ...
    resolver: yupResolver(validationSchemaNatureEq),
    defaultValues: {
      name: "",
    },
  });
  ...
  return (
    <Box sx={{ width: "100%", padding: 3 }}>
      <Typography variant="h5" gutterBottom>
        Ajout d'une nature d'équipement
      </Typography>
      ...
      >
        ...
        >
          Ajouter une nature d'équipement
        </Button>
