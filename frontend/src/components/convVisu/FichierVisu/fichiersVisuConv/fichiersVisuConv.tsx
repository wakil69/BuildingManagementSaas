...
import OverlayRemoveFileConv from "../overlayRemoveFileConv/overlayRemoveFileConv";

export default function FichiersVisuConv({
  files,
  version,
  id,
  latestVersion
  ...
  const addFiles = async (files: FileList) => {
    try {
      ...
      const response = await customRequest.post(
        `${import.meta.env.VITE_APP_API_URL}/convention/files/${id}/${version}`,
        formData
      );
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      >
        <Typography variant="h6">Fichiers de la convention</Typography>
        <motion.div
