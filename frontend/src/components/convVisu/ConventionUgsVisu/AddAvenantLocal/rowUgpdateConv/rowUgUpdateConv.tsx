...
import { LocalAvailable } from "../../../../../types/convention/convention";
...
import { AddAvenantLocalType } from "../../../../convVisu/ConventionUgsVisu/AddAvenantLocal/AddAvenantLocal";

export default function RowUgUpdateConv({
  index,
  errors,
  register,
  getValues,
  ...
  index: number;
  register: UseFormRegister<AddAvenantLocalType>;
  setValue: UseFormSetValue<AddAvenantLocalType>;
  errors: FieldErrors<AddAvenantLocalType>;
  getValues: UseFormGetValues<AddAvenantLocalType>;
  batimentId: number;
  remove: UseFieldArrayRemove;
  watch: UseFormWatch<AddAvenantLocalType>;
  convId?: string;
  version?: string;
}) {
  ...
  async function getLocaux(): Promise<LocalAvailable[]> {
    try {
      const response = await customRequest.get(
        `/convention/locaux-for-update?conv_id=${convId}&version=${version}&dateDebut=${dateDebut}&dateFin=${dateFin}&batiment_id=${batimentId}`
      );
      ...
      if (!getValues(`ugs.${index}.added`)) {
        setValue(
          `ugs.${index}.surface_available`,
          response.data.find((local: LocalAvailable) => local.ug_id === ugId)
            ?.surface_available || 0
        );
        setValue(
          `ugs.${index}.surface`,
