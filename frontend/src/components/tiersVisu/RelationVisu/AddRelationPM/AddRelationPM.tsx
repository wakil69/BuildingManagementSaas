...
export default function AddRelationPM({
  id,
  relations,
}: {
  id?: string;
  ...
  const addRelation = async (data: AddRelationTypePM) => {
    try {
      setMessage("");
      const response = await customRequest.post(
        `/tiers/relations/PM/${id}`,
        data
      );
  ...
  const addRelationMutation = useMutation({
    mutationFn: (data: AddRelationTypePM) => addRelation(data),
    onSuccess: (data) => {
                    ...
                    allPPs
                      .filter(
                        (pp) =>
                          !relations.some(
                            (relation) =>
                              pp.tiepp_id === (relation as Dirigeant).tiepp_id
                          )
                      )
                      .map((pp) => (
                        <ListItem
