...
import OverlayRemoveFileTiers from "../overlayRemoveFileTiers/overlayRemoveFileTiers";

export default function FichiersVisuTiers({
  files,
  qualite,
  id,
}: {
  ...
  const addFiles = async (files: FileList) => {
    try {
      ...
      const response = await customRequest.post(
        `${import.meta.env.VITE_APP_API_URL}/tiers/files/${qualite}/${id}`,
        formData
      );
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      >
        <Typography variant="h6">Fichiers du tiers</Typography>
        <motion.div
