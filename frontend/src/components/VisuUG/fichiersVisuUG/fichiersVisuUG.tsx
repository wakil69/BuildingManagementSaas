...
import OverlayRemoveFile from "../overlayRemoveFileUG/overlayRemoveFileUG";

export default function FichiersVisuUG({
  files,
  ugId,
}: {
  files: FileGlobal[];
  ...
  const [isOpenDeleteFile, setIsOpenDeleteFile] = useState<number | null>(null);
  ...
  const addFiles = async (files: FileList) => {
    try {
      ...
      const response = await customRequest.post(
        `${import.meta.env.VITE_APP_API_URL}/ug/files/${ugId}`,
        formData
      );
  ...
  const addFilesMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ["ug", "files", ugId] });
      setTimeout(() => {
        setMessage("");
      }, 3000);
    },
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      >
        <Typography variant="h6">Fichiers de l'UG</Typography>
        <motion.div
