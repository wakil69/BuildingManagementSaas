...
export default function OverlayRemoveFileUG({
  setIsOpen,
  filename,
  ugId,
}: {
  setIsOpen: React.Dispatch<React.SetStateAction<null | number>>;
  filename: string;
  ugId?: string;
}) {
  ...
  const deleteFile = async (data: { filename: string }) => {
    try {
      setMessage("");
      const response = await customRequest.put(`/ug/files/${ugId}`, data);
